...
var secp256k1 = /*window.nobleSecp256k1 ||*/ require("@dashincubator/secp256k1");

var enc = new TextEncoder();
var MASTER_SECRET = enc.encode("Bitcoin seed");
...
Object.defineProperty(HDKey.prototype, "publicKey", {
  ...
  set: async function (value) {
    ...
    this._identifier = await hash160(await this.publicKey);
    this._fingerprint = this._identifier.slice(0, 4).readUInt32BE(0);
    this._privateKey = null;
  },
});
...
Object.defineProperty(HDKey.prototype, "publicExtendedKey", {
  get: async function () {
    return bs58check.encode(
      serialize(this, this.versions.public, await this.publicKey),
    );
  },
});

HDKey.prototype.derive = async function (path) {
...
HDKey.prototype.deriveChild = async function (index) {
  ...
  if (isHardened) {
    ...
    data = Buffer.concat([await this.publicKey, indexBuffer]);
  }
  ...
  if (this.privateKey) {
    ...
    try {
      ...
      return await this.deriveChild(index + 1);
    }
...
HDKey.prototype.wipePrivateData = function () {
  if (this._privateKey) {
    crypto.getRandomValues(this._privateKey);
  }
  this._privateKey = null;
  return this;
};
...
async function hash160(buf) {
  ...
  var shaAb = await crypto.subtle.digest("SHA-256", buf);
  var sha = new Uint8Array(hashAb);
