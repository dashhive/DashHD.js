...
(function (window, HDKey) {
  ...
  HDKey.create = function (versions) {
      ...
      if (_privateKey) {
        ...
        try {
          ...
          await hd.setPrivateKey(
            await secp256k1.privateKeyTweakAdd(privateKeyCopy, IL),
          );
        ...
        try {
          ...
          await hd.setPublicKey(
            await secp256k1.publicKeyTweakAdd(publicKeyCopy, IL, true),
          );
    ...
    hdkey.sign = async function (hash) {
      ...
      return await secp256k1.ecdsaSign(hash, _privateKey).signature;
    };

    hdkey.verify = async function (hash, signature) {
      return await secp256k1.ecdsaVerify(signature, hash, hdkey.publicKey);
    };
    ...
    hdkey.toJSON = async function () {
      return {
        xpriv: await hdkey.getPrivateExtendedKey(),
        xpub: await hdkey.getPublicExtendedKey(),
      };
    };
