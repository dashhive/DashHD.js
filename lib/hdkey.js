...
(function (window, HDKey) {
  ...
  const COMPRESSED_KEY = true;
  //const BUFFER_LE = true;
  const BUFFER_BE = false;

  //@ts-ignore - global detection failed
  let Crypto = window.crypto || require("crypto");
  //@ts-ignore
  let RIPEMD160 = window.RIPEMD160 || require("@dashincubator/ripemd160");
  //@ts-ignore
  ...
  //@ts-ignore
  //let bs58check = window.Base58Check || require("@dashincubator/base58check");
  ...
  HDKey.create = function (versions) {
    ...
    hdkey.setPrivateKey = async function (value) {
      ...
      hdkey.publicKey = Secp256k1.getPublicKey(_privateKey, COMPRESSED_KEY);
    ...
    hdkey.setPublicKey = async function (value) {
      ...
      try {
        let point = Secp256k1.Point.fromHex(value);
        value = point.toRawBytes(COMPRESSED_KEY);
      } catch (e) {
        throw new Error("Invalid public key");
      }

      await hdkey._setPublicKey(value);
    };
    ...
    hdkey.deriveChild = async function (index) {
      ...
      let I = await createHmac512(hdkey.chainCode, data);
      let IL = I.slice(0, 32);
      let IR = I.slice(32);
      ...
      if (_privateKey) {
        ...
        try {
          ...
          await hd.setPrivateKey(
            // TODO move to Secp256k1
            await tweakUtils.privateAdd(privateKeyCopy, IL),
          );
        ...
        try {
          ...
          await hd.setPublicKey(
            await tweakUtils.pointAddScalar(publicKeyCopy, IL, COMPRESSED_KEY),
          );
    ...
    // TODO: drop sign (best left to the Secp256k1 library
    hdkey.sign = async function (hash) {
      if (!_privateKey) {
    ...
    // TODO: drop sign (best left to the Secp256k1 library
    hdkey.verify = async function (hash, sig) {
    ...
    hdkey.wipePrivateData = function () {
      if (_privateKey) {
        if (Crypto.getRandomValues) {
          Crypto.getRandomValues(_privateKey);
        } else {
  ...
  HDKey.fromMasterSeed = async function (seedBuffer, versions) {
    let I = createHmac512(MASTER_SECRET, seedBuffer);
   ...
   */
  function createHmac512(entropy, data) {
    if (Crypto.createHmac) {
      let hmac = Crypto.createHmac("sha512", entropy);
      hmac.update(data);
      let buf = hmac.digest();
   ...
   * @returns {Uint8Array} - sha256 hash
   */
  function sha256(buf) {
    if (Crypto.createHash) {
      let hash = Crypto.createHash("sha256");
      hash.update(buf);
      let sha256 = hash.digest();
      let u8 = new Uint8Array(sha256);
      return u8;
    }
    // TODO WebCrypto
    throw new Error("not implemented");
