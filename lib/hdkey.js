...
(function (window, HDKey) {
      ...
      if (_privateKey) {
        ...
        try {
          await hd.setPrivateKey(
            Buffer.from(
              secp256k1.privateKeyTweakAdd(Uint8Array.from(_privateKey), IL),
            ),
          );
  ...
  HDKey.fromJSON = async function (obj) {
    return await HDKey.fromExtendedKey(obj.xpriv);
  };
