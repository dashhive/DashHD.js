...
Utils.encodeXPrv = async function (keyBytes, opts) {
  ...
  //@ts-expect-error - TODO DashKeys opts.version is too restrictive
...
Utils.privateKeyTweakAdd = async function (privateKeyCopy, tweak) {
  ...
  let Secp256k1 =
    //@ts-expect-error - if it exists
    globalThis.nobleSecp256k1 || (await import("@dashincubator/secp256k1"));
...
DashHd.HARDENED_OFFSET = 0x80000000;
let KEY_SIZE = 33;
...
DashHd.toXPrv = async function (hdkey, opts) {
  if (!hdkey?.privateKey) {
    throw new Error("missing .privateKey bytes");
  }
...
DashHd.deriveChild = async function (hdkey, index, hardened = DashHd.HARDENED) {
  // seed = indexedKey
...
DashHd.derivePath = async function (parent, path) {
  ...
  for (let i = 0; i < entries.length; i += 1) {
    ...
    assert(childIndex < DashHd.HARDENED_OFFSET, "Invalid index");
...
DashHd._createAccount = async function (accountKey) {
  ...
  async function deriveXKey(use = DashHd.RECEIVE) {
    let xkey = await DashHd.deriveChild(accountKey, use, DashHd.PUBLIC);
...
DashHd._createXKey = async function (xkey) {
  ...
  async function deriveAddress(index) {
    let key = await DashHd.deriveChild(xkey, index, DashHd.PUBLIC);
    return key;
  }
...
DashHd.fromXKey = async function (xkey, opts) {
  ...
  /** @type {String} */ //@ts-expect-error - TODO update DashKeys types
  let keyHex = keyInfo.xprv || keyInfo.xpub;
  let keyBytes = DashKeys.utils.hexToBytes(keyHex);
...
export default DashHd; // for ESM compatibility
// for NPM/CommonJS compatibility with node 22.0+
export let CHANGE = DashHd.CHANGE;
export let HARDENED = DashHd.HARDENED;
export let HARDENED_OFFSET = DashHd.HARDENED_OFFSET;
export let MAINNET = DashHd.MAINNET;
export let PUBLIC = DashHd.PUBLIC;
export let RECEIVE = DashHd.RECEIVE;
export let TESTNET = DashHd.TESTNET;
// export let _createAccount = DashHd._createAccount;
// export let _createXKey = DashHd._createXKey;
// export let _derive = DashHd._derive;
// export let _fingerprint = DashHd._fingerprint;
// export let _toXBytes = DashHd._toXBytes;
// export let _utils = DashHd._utils;
export let create = DashHd.create;
export let deriveChild = DashHd.deriveChild;
export let derivePath = DashHd.derivePath;
export let fromSeed = DashHd.fromSeed;
export let fromXKey = DashHd.fromXKey;
export let toAddr = DashHd.toAddr;
export let toId = DashHd.toId;
export let toIdBytes = DashHd.toIdBytes;
export let toPublic = DashHd.toPublic;
export let toWif = DashHd.toWif;
export let toXPrv = DashHd.toXPrv;
export let toXPrvBytes = DashHd.toXPrvBytes;
export let toXPub = DashHd.toXPub;
export let toXPubBytes = DashHd.toXPubBytes;
export let wipePrivateData = DashHd.wipePrivateData;
